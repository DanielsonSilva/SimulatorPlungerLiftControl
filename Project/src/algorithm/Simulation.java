...
public class Simulation {
	...
	void EquacoesAuxiliares::subidaPistao(){
		...
		this->enviarVarCiclo(STAGE_RISE_DURATION,(--v->i/(1.0/c->step)+v->transient));
	...
	void EquacoesAuxiliares::producaoLiquido(){
		...
		enviarVarCiclo(STAGE_PRODUC_DURATION, v->j * c->step_);
	...
	void EquacoesAuxiliares::Controle(){
	  ...
	  //PRODUÇÃO ACUMULADA
		...
		this->enviarVarCiclo(PRODUCTION_VOLUME, v->production);
		//ENVIAR O DADO DE PRODUCAO DO POCO PARA INTERFACE
		this->enviarVarCiclo(TOTAL_PRODUCTION, v->total_production);
		//ENVIAR O DADO DE TEMPO DE CHEGADA DO PISTAO NA SUPERFICIE
		this->enviarVarCiclo(PL_RISE_TIME, v->piston_arrival);
		...
		if( v->piston_arrival != 0 ){
			...
			this->enviarVarCiclo(AVERAGE_PL_VELOCITY, v->velocity);
			//DADO DE VELOCIDADE DE IMPACTO
			this->enviarVarCiclo(IMPACT_VEL, v->v0);
		}
	...
	void EquacoesAuxiliares::Afterflow(){
		...
		enviarVarCiclo(STAGE_AFTER_DURATION, v->k * c->step_aft);
...
//		v->limite = (f->tubing->Lcauda - f->tempos->Ltbg) * (60*3.2808)/ f->pistao->Vqpg + ((v->Ppart_csg - v->Ppart_tbg) * (60*3.2808)/(c->ROliq * c->G * f->pistao->Vqpl));
		...
		enviarVarCiclo(STAGE_BUILDUP_DURATION, c->_step * v->m);
